...
from math import dist, radians, cos, sin, atan2, degrees
...
class Ball:
    ...
    def update(self):
        if self.x + self.radius > self.window.get_width() or self.x - self.radius < 0:
            self.vector.end_pos[0] *= -1
        if self.y + self.radius > self.window.get_height() or self.y - self.radius < 0:
